...
{
    /// <summary>
    /// Расширенная информация о контакте, приходящая в SNAC(03,0B)
    /// (User Online) и SNAC(03,0F) (User Status Change).
    ///
    /// Базовый набор полей — как в исходном ContactInfo.cs
    /// (без [DataContract]/[DataMember], namespace не указан).
    /// Для поддержки QIP-статусов и X-Status добавлены:
    ///   QipStatus           — финальный QIP-код (0x3000/0x4000/0x5000/…)
    ///   XStatusId           — 0..36 (0 = нет X-Status)
    ///   XStatusTitle        — "Злой", "Дома"…
    ///   XStatusDescription  — "Я дома"
    ///   Capabilities        — все 16-байтные GUID, пришедшие в TLV 0x000D
    /// </summary>
    public class ContactInfo
    {
        ...
        // ── Новые поля для QIP / X-Status ──────────────────────────────────
        /// <summary>
        /// Код QIP-статуса, которым был ПЕРЕЗАПИСАН базовый OSCAR-статус
        /// (например, 0x3000 = Evil, 0x5000 = At Home). 0 = QIP-статус
        /// не выставлен, используется обычный OSCAR-статус из Status.
        /// </summary>
        public uint QipStatus { get; set; }

        /// <summary>
        /// Идентификатор X-Status (0..36). 0 = X-Status не выставлен.
        /// </summary>
        public int XStatusId { get; set; }

        /// <summary>
        /// Локализованное название X-Status ("Сон", "Дома"…).
        /// </summary>
        public string XStatusTitle { get; set; }

        /// <summary>
        /// Описание X-Status ("Сплю", "Я дома"…).
        /// </summary>
        public string XStatusDescription { get; set; }

        /// <summary>
        /// Список GUID всех capability, пришедших в TLV 0x000D
        /// (hex-строки в верхнем регистре). Нужен для бинарной
        /// совместимости с QIP-клиентами.
        /// </summary>
